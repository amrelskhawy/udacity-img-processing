    ...
    describe('Status Codes tests', () => {
        ...
        it('expects to return 400', async () => {
            const response = await request.get(
                '/api/images?filename=keyboard.jpg&width=500&height=500'
            );
        ...
        it('expectd to bad request', async () => {
            const response = await request.get(
                '/api/images?filename=keyboard&width=500&height=500'
            );
        ...
        it('expectd to be 400', async () => {
            const response = await request.get(
                '/api/images?filename=keyboard&width'
            );
        ...
        it('Expected new file exists', async () => {
            ...
            const res = fs.existsSync(
                path.resolve('./') + '/assets/thumbs/keyboard.jpg-500_500.jpg'
            );
            expect(res).toBeTrue();
        });
        ...
        it('Expected new file exists', async () => {
            const response = await request.get(
                '/api/images?filename=keyboard.jpg&width=&height='
            );
